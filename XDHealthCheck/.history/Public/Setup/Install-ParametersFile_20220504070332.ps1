...
function Install-ParametersFile {
	...
	$ParPath = Join-Path -Path $ParametersFolder -ChildPath ""
	if (Test-Path -Path ) { Rename-Item "$ParametersFolder\Parameters.json" -NewName "Parameters_$(Get-Date -Format ddMMyyyy_HHmm).json" }
