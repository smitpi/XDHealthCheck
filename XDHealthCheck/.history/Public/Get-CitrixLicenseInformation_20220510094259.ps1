...
Function Get-CitrixLicenseInformation {
	...
	if ($Export -eq 'Excel') { 
		$LicDetails | Export-Excel -Path $(Join-Path -Path $ReportPath -ChildPath "\CitrixLicenseInformation-$(Get-Date -Format yyyy.MM.dd-HH.mm).xlsx") -WorksheetName CitrixLicenseInformation -AutoSize -AutoFilter -Title 'Machine Failures' -TitleBold -TitleSize 28
	}
	if ($Export -eq 'HTML') { 
