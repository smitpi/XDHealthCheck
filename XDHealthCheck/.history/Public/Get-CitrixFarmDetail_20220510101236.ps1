...
Function Get-CitrixFarmDetail {
	...
	if ($Export -eq 'Excel') {  
		...
		$SessionCounts | Export-Excel -Path $(Join-Path -Path $ReportPath -ChildPath "\CitrixFarmDetail-$(Get-Date -Format yyyy.MM.dd-HH.mm).xlsx") -WorksheetName SessionCounts -AutoSize -AutoFilter -Title 'Machine Failures' -TitleBold -TitleSize 28
		...
		$Failures | Export-Excel -Path $(Join-Path -Path $ReportPath -ChildPath "\CitrixFarmDetail-$(Get-Date -Format yyyy.MM.dd-HH.mm).xlsx") -WorksheetName Failures -AutoSize -AutoFilter -Title 'Machine Failures' -TitleBold -TitleSize 28
		$appver | Export-Excel -Path $(Join-Path -Path $ReportPath -ChildPath "\CitrixFarmDetail-$(Get-Date -Format yyyy.MM.dd-HH.mm).xlsx") -WorksheetName AppVersions -AutoSize -AutoFilter -Title 'Machine Failures' -TitleBold -TitleSize 28
		$CitrixSessionIcaRtt | Export-Excel -Path $(Join-Path -Path $ReportPath -ChildPath "\CitrixFarmDetail-$(Get-Date -Format yyyy.MM.dd-HH.mm).xlsx") -WorksheetName CitrixSessionIcaRtt -AutoSize -AutoFilter -Title 'Machine Failures' -TitleBold -TitleSize 28
	}
	if ($Export -eq 'HTML') { 
