...
Function Get-CitrixFarmDetail {
	...
	if ($Export -eq 'HTML') { 
		...
		New-HTML -TitleText "CitrixFarmDetail-$(Get-Date -Format yyyy.MM.dd-HH.mm)" -FilePath $(Join-Path -Path $ReportPath -ChildPath "\CitrixFarmDetail-$(Get-Date -Format yyyy.MM.dd-HH.mm).html") {
			...
			New-HTMLTab -Name 'Sessions' -TextTransform uppercase -IconSolid cloud-sun-rain -TextSize 16 -TextColor '#00203F' -IconSize 16 -IconColor '#ADEFD1' -HtmlData {New-HTMLPanel -Content { New-HTMLTable -DataTable $($CTXObject.'Per Device') @TableSettings}}
