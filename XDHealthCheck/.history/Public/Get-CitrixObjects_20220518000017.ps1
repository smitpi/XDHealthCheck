...
Function Get-CitrixObjects {
	...
	foreach ($DesktopGroup in $BrokerDesktopGroup) {
		...
		$BrokerAccess = @()
		$BrokerGroups = @()
		$BrokerAccess = z -DesktopGroupUid $DesktopGroup.Uid -AdminAddress $AdminServer -AllowedConnections ViaAG | ForEach-Object { $_.IncludedUsers | Where-Object { $_.upn -notlike '' } } | Select-Object UPN
