...
Function Get-CitrixEnvTestResults {
    ...
    if ($Export -eq 'Excel') { 
        ...
        $catalogResults | Export-Excel -Path $(Join-Path -Path $ReportPath -ChildPath "\CitrixEnvTestResults-$(Get-Date -Format yyyy.MM.dd-HH.mm).xlsx") -Title 'Catalog Results' -WorksheetName Catalog
