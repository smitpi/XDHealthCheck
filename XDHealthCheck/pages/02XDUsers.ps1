...
New-UDCollapsibleItem  -Endpoint {
	    ...
	    $validuser = Get-FullUserDetail -UserToQuery $username  -DomainFQDN 'corp.dsarena.com' -DomainCredentials $CTXAdmin -RunAsPSRemote
...
New-UDCollapsibleItem -Endpoint {
	New-UDInput -Title "Compare Users" -Content {
		...
		$compareUsers = Compare-TwoADUser -Username1 $Username1 -Username2 $Username2 -Verbose
...
New-UDCollapsibleItem  -Endpoint {
	...
	New-UDLayout -Columns 4 -Content {
		...
		New-UDGrid -Title 'Current Applications' -Endpoint { ($UserDetail.AccessPublishedApps | Select-Object PublishedName, Description, enabled) | Out-UDGridData }
		New-UDGrid -Title 'Current Desktops' -Endpoint { $DesktopsCombined | Out-UDGridData }
		New-UDGrid -Title 'Available Applications' -Endpoint { ($UserDetail.NoAccessPublishedApps | Select-Object PublishedName, Description, enabled) | Out-UDGridData }
	}
)
}
