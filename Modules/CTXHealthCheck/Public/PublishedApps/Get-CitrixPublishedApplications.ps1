...
Write-Verbose "$((Get-Date -Format HH:mm:ss).ToString()) [Processing] Delivery Group: $($item.name.ToString())"
	...
	[string]$usercsv = $BrokerAccess | foreach {$usercsv + $_.upn + ','}
    [string]$groupscsv = $BrokerGroups | foreach {$groupscsv + $_.fullname + ','}
...
foreach ($DeskG in ($ReadAbleDG | where { $_.DeliveryType -like 'DesktopsAndApps' })) {
	...
	foreach ($PublishedApp in $PublishedApps) {
        ...
        [string]$userappscsv = $PublishedAppUser | foreach {$userappscsv + $_ + ','}
        [string]$groupsappscsv = $PublishedAppGroup | foreach {$groupsappscsv + $_ + ','}
		...
		$CusObject = New-Object PSObject -Property @{
            ...
            DesktopGroupUsersCSV     = $DeskG.IncludeADGroupsCSV
			...
			PublishedAppGroupCSV     = $groupsappscsv
			PublishedAppUserCSV      = $userappscsv
...
else { $AppDetail = GetAllConfig -AdminAddress $AdminServer -VerbosePreference $VerbosePreference  }
